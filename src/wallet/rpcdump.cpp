...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    "as change, and not show up in many RPCs.\n",
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Sparks address (or hex-encoded script)"},
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Sparks address or script");
        }
    }
    if (fRescan)
    {
...
UniValue importprunedfunds(const JSONRPCRequest& request)
{
    ...
    LOCK(pwallet->cs_wallet);
    int height;
    if (!pwallet->chain().findAncestorByHash(pwallet->GetLastBlockHash(), merkleBlock.header.GetHash(Params().GetConsensus()), FoundBlock().height(height))) {
    ...
    CWalletTx::Confirmation confirm(CWalletTx::Status::CONFIRMED, height, merkleBlock.header.GetHash(Params().GetConsensus()), txnIndex);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see sparksd log for details) and could "
