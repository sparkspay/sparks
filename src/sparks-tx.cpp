...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s sparks-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  sparks-tx [options] <hex-tx> [commands]  Update hex-encoded sparks transaction\n" +
              "  sparks-tx [options] -create [commands]   Create hex-encoded sparks transaction\n" +
              "\n";
