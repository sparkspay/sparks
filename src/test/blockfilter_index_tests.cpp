    ...
    const CScript& scriptPubKey)
{
    ...
    while (!CheckProofOfWork(block.GetHash(chainparams.GetConsensus()), block.nBits, chainparams.GetConsensus())) ++block.nNonce;
...
BOOST_FIXTURE_TEST_CASE(blockfilter_index_initial_sync, BuildChainTestingSetup)
{
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        {
            LOCK(cs_main);
            block_index = g_chainman.m_blockman.LookupBlockIndex(block->GetHash(Params().GetConsensus()));
        }
    ...
    // Reorg back to chain A.
     ...
     for (size_t i = 0; i < 3; i++) {
         ...
         {
             LOCK(cs_main);
             block_index = g_chainman.m_blockman.LookupBlockIndex(chainA[i]->GetHash(Params().GetConsensus()));
         }
         BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
         CheckFilterLookups(filter_index, block_index, chainA_last_header);
         ...
         {
             LOCK(cs_main);
             block_index = g_chainman.m_blockman.LookupBlockIndex(chainB[i]->GetHash(Params().GetConsensus()));
         }
         BOOST_CHECK(filter_index.BlockUntilSyncedToCurrentChain());
         CheckFilterLookups(filter_index, block_index, chainB_last_header);
     }
