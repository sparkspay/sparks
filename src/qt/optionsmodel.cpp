...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SPK);
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeSparksAmount"))
            ...
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizeSparksAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sparks.cpp during shotdown to also remove the window positions
