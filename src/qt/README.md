This directory contains the source code for the Sparks Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Sparks UI.
...
- ClientModel is used by the main application `sparksgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoin/bitcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a sparks:... link from a browser).
...
**Note:** Do not change `local/sparks_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `sparks-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `sparks-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `sparks-qt`
9. You're all set! Start developing, building, and debugging the Sparks Core GUI
