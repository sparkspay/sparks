                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    uint256 block_hash{block.GetHash(GetBlockAlgo(Params().GetConsensus()))};
    ...
    do {
...

        {
            ...
            do {
                ...
                if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash(GetBlockAlgo(Params().GetConsensus())) == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace)) {
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlockHeader(block, block.GetHash(GetBlockAlgo(consensusParams)), state, consensusParams, fCheckPOW))
        return false;
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash(GetBlockAlgo(chainparams.GetConsensus()));
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    uint256 block_hash(block.GetHash(GetBlockAlgo(chainparams.GetConsensus())));
...
bool CChainState::AddGenesisBlock(const CBlock& block, BlockValidationState& state)
{
    ...
    CBlockIndex* pindex = m_blockman.AddToBlockIndex(block, block.GetHash(GetBlockAlgo(m_params.GetConsensus())));
    ReceivedBlockTransactions(block, pindex, blockPos);
    return true;
}
...
bool CChainState::LoadGenesisBlock()
{
    ...
    if (m_blockman.m_block_index.count(m_params.GenesisBlock().GetHash(BlockAlgo::NEOSCRYPT)))
        return true;
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                uint256 hash = block.GetHash(GetBlockAlgo(m_params.GetConsensus()));
                {
                    LOCK(cs_main);
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, m_params.GetConsensus())) {
                            LogPrint(BCLog::REINDEX, "%s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHash(GetBlockAlgo(m_params.GetConsensus())).ToString(),
                                    head.ToString());
                            LOCK(cs_main);
                            assert(std::addressof(::ChainstateActive()) == std::addressof(*this));
                            ...
                            if (AcceptBlock(pblockrecursive, dummy, nullptr, true, &it->second, nullptr)) {
                                nLoaded++;
                                queue.push_back(pblockrecursive->GetHash(GetBlockAlgo(m_params.GetConsensus())));
                            }
                        }
                        range.first++;
...
void CChainState::CheckBlockIndex()
{
    ...
    while (pindex != nullptr) {
        ...
        // assert(pindex->GetBlockHash() == pindex->GetBlockHeader().GetHash(GetBlockAlgo(m_params.GetConsensus()))); // Perhaps too slow
