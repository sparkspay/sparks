...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for sparksd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if sparksd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('sparksd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: sparksd started, waiting for RPC to come up")
    ...
    for i in range(num_nodes):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in sparks.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start a sparksd and return RPC connection to it
    """
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start multiple sparksds, return RPC connections to them
    """
