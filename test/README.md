This directory contains integration tests that test sparksd and its
...
sparksd and sparks-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the sparks utilities, currently only
sparks-tx.
...
Before tests can be run locally, Sparks Core must be built.  See the [building instructions](/doc#building) for help.
...
Many Sparks specific tests require sparks_hash. To install it:

- Clone the repo `git clone https://github.com/sparkspay/sparks_hash`
- Install sparks_hash `cd sparks_hash && pip3 install -r requirements.txt .`
...
```
test/get_previous_releases.py -b v19.3.0 v18.2.3 v0.17.3.0 v0.16.0.0 v0.15.0.0
```
...
The P2P and RPC ports used by the sparksd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another sparksd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its sparksd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other sparksd processes are running.

On linux, the test framework will warn if there is another
sparksd process running when the tests are started.

If there are zombie sparksd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
sparksd processes running on the system, so should not be used if any non-test
sparksd processes are being run.**
...
sure sparksd processes are stopped as above):
...
`test_framework.log` and sparksd `debug.log`s can be combined into a single
...
call methods that interact with the sparksd nodes-under-test.

If further introspection of the sparksd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/sparksd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/sparksd <pid>
```
