# Fuzzing Sparks Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Sparks Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
Every single pull request submitted against the Sparks Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Sparks Core more robust.
...
with certain assembly code from Sparks Core's code. See [developer notes on sanitizers](https://github.com/sparkspay/sparks/blob/develop/doc/developer-notes.md#sanitizers)
...
To quickly get started fuzzing Sparks Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Sparks Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Sparks
Core without having to write any custom fuzzing harness. The `sparksd` server
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/sparksd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
