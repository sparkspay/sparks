...
Currently, 10% of block rewards are set aside for the Sparks DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Sparks in a
credit pool. This credit pool mirrors the available credit supply in Sparks
Platform. When a certain amount of Sparks is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Sparks
Platform. Masternode identities receive the fees and can convert them back to
Sparks by withdrawing them from Sparks Platform.
...
activation and a newly introduced `SPORK_25_TEST_EHF` spork.
...
### Removal of reject network messages from Sparks Core (BIP61)

This feature has been disabled by default since Sparks Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Sparks Core version 20.0.0 there are extra changes:
...
- It is possible to run Sparks Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `Sparks/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Sparks Core (BIP61)_](#removal-of-reject-network-messages-from-sparks-core-bip61) above for
...
- The `sparks-cli -getinfo` command now displays the wallet name and balance for
...
python test framework and `sparks-cli` tool both take advantage of this, so
for example:

```sh
sparks-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
sparks-cli -named createwallet mywallet load_on_startup=1
```
...
with Sparks’ product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/sparkspay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/sparkspay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/sparkspay/sparks/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/sparkspay/sparks/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/sparkspay/sparks/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/sparkspay/sparks/compare/v19.3.0...sparkspay:v20.0.0
