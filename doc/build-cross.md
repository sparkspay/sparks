Cross-compiliation of Sparks Core
===============================

Sparks Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Sparks.
...
It's not possible to build the Sparks Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Sparks Core source code).
